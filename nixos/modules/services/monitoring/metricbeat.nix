...
in
{
  options = {
    ...
    services.metricbeat = {
      ...
      modules = mkOption {
        description = ''
          Metricbeat modules are responsible for read metrics from the various sources.

          This like <literal>services.metricbeat.settings.metricbeat.modules</literal>,
          ...
          for each, but setting the same <xref linkend="opt-services.metricbeat.modules._name_.module"/>.
